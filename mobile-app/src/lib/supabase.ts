...
const MOBILE_ROLES: RoleName[] = ['student', 'coordinator', 'supervisor']
...
export function isMobileAllowedProfile(profile: Profile | null): boolean {
  const role = profileRoleName(profile)
  return role !== null && MOBILE_ROLES.includes(role as RoleName)
}

export function getMobileDashboardRole(profile: Profile | null): RoleName | null {
  const role = profileRoleName(profile)
  if (!role || !MOBILE_ROLES.includes(role as RoleName)) return null
