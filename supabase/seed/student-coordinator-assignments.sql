-- Student ↔ coordinator assignments
-- Run in Supabase SQL Editor after roles, profiles, and school_years exist.
...
create policy "Coordinators read their assignments"
  ...
  for select
  to authenticated
  using (coordinator_id = public.current_profile_id());
...
-- ---------------------------------------------------------------------------
-- Drop school_year_id from profiles
-- ---------------------------------------------------------------------------
...
-- Coordinators need to read assigned student profiles (mobile app student list)
...
-- Example assign (replace IDs):
-- select public.assign_student_coordinator(
--   p_student_id := 3,
--   p_coordinator_id := 2,
--   p_school_year_id := 1
-- );
